...
def normalize_case_package(package: dict[str, Any]) -> dict[str, Any]:
    ...
    # 早期通用草稿曾预置三个人名，而发布门禁同时禁止这些模板人物，形成
    # “上游注入、下游必拒”的契约冲突。统一迁移为角色称谓，也更符合事实审慎要求。
    generic_roles = {
        "陈启明": "项目总负责人",
        "林晓雯": "专业协调负责人",
        "赵磊": "一线执行负责人",
    ...
    def replace_generic_roles(value: Any) -> Any:
        if isinstance(value, str):
            for old, new in generic_roles.items():
                value = value.replace(old, new)
            return value
        if isinstance(value, list):
            return [replace_generic_roles(item) for item in value]
        if isinstance(value, dict):
            return {key: replace_generic_roles(item) for key, item in value.items()}
        return value
    ...
    for key in (
        ...
        if key in package:
            package[key] = replace_generic_roles(package[key])
    body = package.setdefault("body", {})
...
def ensure_mock_body_length(package: dict[str, Any], task: CaseTask) -> int:
    ...
    segments = [
        ...
        "项目总负责人在首次启动会上展示了清晰的里程碑：第一个月完成准备，第二个月进入试运行，第三个月提交阶段成果。他强调，组织已经错过过一次窗口期，如果再次延后，不仅预算可能被收回，团队也会失去高层支持。会议记录显示，多数参会者当场表示同意，但会后提交的风险清单却不断增加。",
        "专业协调负责人负责跨部门协调。她很快发现，表面上的共识掩盖了完全不同的理解：决策层把任务视为战略落地，职能部门把它视为流程调整，一线员工则把它理解为额外工作。三方使用相同的项目名称，却在讨论不同的问题。她尝试建立共同的问题清单，但每个部门都更关心自身指标是否会受到影响。",
        "一线执行负责人代表一线团队参加第二次协调会。他没有否定项目方向，而是列出最近四周的排班、返工和客户反馈数据，说明团队已接近负荷上限。如果在培训和岗位安排尚未完成时直接切换流程，短期指标也许会改善，但错误成本会被转移给最靠近客户的人。他的发言让会议气氛第一次变得紧张。",
        "项目总负责人认为这些问题可以在推进过程中逐步解决。他担心过度讨论会让组织重新回到观望状态，因此要求各部门先执行统一方案，再根据数据调整。专业协调负责人则提出，执行不是简单服从；如果关键参与者不知道改变与自身工作的关系，再完整的计划也可能只停留在汇报材料中。双方的分歧从进度安排逐渐转向对“有效执行”的不同理解。",
        ...
        "专业协调负责人分别与关键成员进行访谈。年轻员工普遍愿意尝试，但担心犯错会影响考核；资深员工并非抗拒变化，而是认为既有经验被轻易否定；中层管理者最矛盾，他们既要向上证明执行力度，又要承担团队波动带来的现实后果。这些信息没有直接给出答案，却说明所谓“阻力”并不是单一态度问题。",
        f"在《{course}》所关注的组织情境中，正式权力、专业判断和非正式影响同时发挥作用。项目总负责人拥有资源配置权，一线执行负责人掌握一线经验，专业协调负责人则连接了原本彼此隔离的信息。任何一方单独推进都可能获得局部成功，却难以形成稳定结果。问题的核心逐渐从选择某个工具，转变为如何建立共同承担结果的机制。",
        "第三次项目会议前，高层发来通知，要求提前两周展示成果。项目总负责人据此主张扩大试点范围，用更明显的数据证明项目价值。一线执行负责人反对在问题尚未闭环时扩大规模，认为这会把局部风险快速复制。专业协调负责人提出折中方案：保持总体目标不变，但重新划分试点边界，并用一周时间完成关键岗位训练和责任确认。",
        ...
        "会前一天，一线执行负责人提交了一份联合签名的建议书。建议书没有要求暂停项目，而是要求公开试点数据、明确异常处理权限，并允许一线成员参与每周复盘。项目总负责人意识到，如果接受这些要求，原计划必须调整；如果拒绝，项目仍可按时推进，但一线可能只做最低限度的配合。形式上的速度与长期的执行质量形成直接冲突。",
        ...
        "最终协调会开始时，项目总负责人没有立即公布决定，而是把三个方案写在白板上：维持原计划并扩大试点；暂缓扩围、集中补齐训练与责任机制；重新设定阶段目标，以较慢速度换取更多参与。会议室里的每个人都知道，选择任何一项都意味着放弃另一些价值，而决定必须在当天作出。",
    ]
    ...
    while count_case_body_chars(package) < min_chars:
        ...
        narrative_parts.append(
            f"在第{index + 1}轮补充讨论中，团队把“{topic}”单独列为判断维度。现有材料表明，{evidence}；但如果立即修正，{tradeoff}。项目总负责人要求用结果证明必要性，专业协调负责人关注不同部门能否形成共同规则，一线执行负责人则追问风险最终由谁承担。这一轮讨论没有消除分歧，却让各方必须公开自己的判断标准。"
        )
        body["narrative"] = "\n\n".join(narrative_parts)
        index += 1
...
def build_structured_package(task: CaseTask, *, domain_notes: str | None = None) -> dict[str, Any]:
    ...
    characters = [
        {"name": "项目总负责人", "role": "决策者/负责人", "stance": "倾向推进既定方案"},
        {"name": "专业协调负责人", "role": "中层协调者", "stance": "呼吁兼顾执行层反馈"},
        {"name": "一线执行负责人", "role": "一线代表", "stance": "担忧节奏与资源不足"},
    ]
    material_query = f"{title} {subject} {course} {task.case_type}"
    ...
    package: dict[str, Any] = {
        ...
        "body": {
            ...
            "narrative": (
                ...
                "项目总负责人强调窗口期与目标承诺，要求按原计划推进；"
                "专业协调负责人整理了跨部门反馈，指出沟通不足与节奏过快已引发抵触；"
                "一线执行负责人代表执行层提出资源与能力准备不足，若强行推进可能影响质量与士气。"
        ...
        "discussion_questions": [
            ...
            {
                "level": "分析",
                "question": f"结合{subject}理论，如何解释专业协调负责人和一线执行负责人的立场差异？",
