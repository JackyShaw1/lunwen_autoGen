...
系统内置自托管 SearXNG 自动资料研究，默认优先使用百度、360 搜索、搜狗和夸克，并保留 Bing 与 DuckDuckGo 作为备用引擎。教师要求真实案例且本地没有审核资料包时，系统自动执行中文课程检索、来源过滤与去重，建立最多 10 条带证据片段的临时事实包，再要求模型用 `[S1]…` 引用生成；教师无需自行搜集来源，也无需配置第三方搜索 API Key。
...
| GET | `/api/admin/research-config` | 内置自动事实研究状态（管理员） |
| POST | `/api/admin/research-config/test` | 测试内置检索服务（管理员） |
